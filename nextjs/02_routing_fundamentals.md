...
```tsx
// app/blog/[slug]/page.tsx (Next 15)
...
This is one of the App Router's most powerful and most confusing features. Build it once, you'll never forget it; read about it without building it, and you will.
...
`searchParams` makes a page **dynamically rendered** by default (it can't be statically prerendered because the query is unknown at build time). See Module 06 for the implications.
...
Navigate around — observe in DevTools how only the changed segment refetches, and the versions sidebar stays mounted. That's the architectural payoff of nested layouts.
...
Layouts **persist** across navigations within the same segment subtree. Templates **re-mount** on every navigation. Use templates for per-navigation animations or `useEffect` reset behavior. Default to layouts; reach for templates rarely.
...
The two APIs have *similar names but different shapes*. App-Router's `useRouter` has no `router.query`; use `useSearchParams()` and `useParams()` instead.
...
`app/blog/[slug]/` and `app/blog/featured/` can coexist, but `/blog/featured` will match the static folder, not the dynamic one — static wins. Inverting that intuition (assuming dynamic absorbs everything) creates real bugs.

### `searchParams` and the static cache

Touching `searchParams` (or any "dynamic API" like `cookies()`, `headers()`) opts a page into dynamic rendering. If you wanted prerendering, either move that read deeper into a Suspense boundary or use Partial Prerendering (Module 06).

### Async `params` in Next 15

Mentioned in Module 01, repeated here because it bites in every routing exercise: `params` and `searchParams` are Promises in Next 15. The function must be `async` and you must `await` them.
...
- **Folders are segments, files are roles.** Internalize the special filenames (`page`, `layout`, `loading`, `error`, `not-found`, `route`, `template`, `default`) — they are your entire routing vocabulary.
- **Layouts persist; templates remount.** Choose deliberately. Most of the time, layouts are right.
- **Route groups `(name)` and private folders `_name` shape your code without shaping the URL.** Use them aggressively for organization.
- **Parallel and intercepting routes are the App Router's superpowers** for modals, split views, and contextual UIs. They're confusing on paper and obvious once you build one.
- **`useRouter` lives in `next/navigation` in the App Router**, and `params`/`searchParams` are Promises in Next 15. Two small details that cause an outsized fraction of bugs — burn them in.
